---
title: "4.0 Conditional Logic"
weight: 4
sidebar:
  open: true
---

Conditional logic lets a program make decisions. This topic covers the structures Java provides for branching program flow based on conditions.

## Learning Objectives

- Implement decision-making structures (if, if-else, switch)
- Apply Boolean logic and comparison operators
- Analyze program flow using conditional structures

## Subtopics

{{< cards >}}
  {{< card link="if-and-if-else" title="if and if-else" subtitle="Basic branching and two-way decisions" >}}
  {{< card link="switch-statements" title="Switch Statements" subtitle="Multi-way branching with switch" >}}
  {{< card link="boolean-logic" title="Boolean Logic" subtitle="Comparison operators, &&, ||, and !" >}}
{{< /cards >}}
