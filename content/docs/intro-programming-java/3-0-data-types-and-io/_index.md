---
title: "3.0 Data Types and Input/Output"
weight: 3
sidebar:
  open: true
---

Programs need to store and manipulate data. This topic covers Java's built-in data types, how to perform calculations, and how to interact with the user through input and output.

## Learning Objectives

- Use primitive data types and variables
- Perform arithmetic operations and calculations
- Implement user input and formatted output

## Subtopics

{{< cards >}}
  {{< card link="primitive-data-types" title="Primitive Data Types" subtitle="int, double, char, boolean, and more" >}}
  {{< card link="arithmetic-and-operators" title="Arithmetic and Operators" subtitle="Math operations and operator precedence" >}}
  {{< card link="input-and-output" title="Input and Output" subtitle="Scanner for input, System.out for output" >}}
{{< /cards >}}
